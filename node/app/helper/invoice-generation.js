...
/*******************added for probe api related files **********/
...
const generateInvoice = {
    ...
    report_cin: function (dataObj) {
        ...
        var creditRatingList = []; var creditRatingKeys = []; var directorNetwork = [];
        ...
        if(dataObj.director_network.length>0)
        {
            dataObj.director_network.map(function (el6) {

                var getCompanyWithoutCessation =[]; var getLlpWithoutCessation=[];
                if(el6.network.companies.length>0)
                {
                    getCompanyWithoutCessation = el6.network.companies.filter(function (value1) {
                        return value1.date_of_cessation==null;
                    });
                }

                if(el6.network.llps.length>0)
                {
                    getLlpWithoutCessation = el6.network.llps.filter(function (value2) {
                        return value2.date_of_cessation==null;
                    });
                }

                if(getCompanyWithoutCessation.length>0 || getLlpWithoutCessation.length>0)
                {
                    directorNetwork.push({
                        name:el6.name,
                        pan:el6.pan,
                        din:el6.din,
                        network:{
                            companies:getCompanyWithoutCessation,
                            llps:getLlpWithoutCessation
                        }
                    })
                }
        ...
        dataObj.directorNetwork = directorNetwork;
    ...
    exceltohtml: function (file_name, online_search_id) {
     ...
     ///////Report for cIN company to get json
     report_cin_test: function (dataObj) {
        //console.log(dataObj,'dataObj');
        var chargeSequence = []; var casesFiledByCorporatesPending = []; var casesFiledByCorporatesDisposed = [];
        var consolidationOfCorporateAffairsPending = []; var consolidationOfCorporateAffairsDisposed = [];
        var casesFiledAgainstCorporatesPending = []; var casesFiledAgainstCorporatesDisposed = [];
    ...
    ///////Report for LLP company to get json
    report_llp_test: function (dataObj) {
        //console.log(dataObj,'dataObj');
    ...
    /**************common fn to fetch comprehensive data and save html CIN */
    cin_comprehensive_report: async function (company_id,user_id,search_investigation_id,online_search_id='',last_updated_date='') 
    {
        // console.log(company_id);
        // console.log(user_id);
        // console.log(search_investigation_id);
        // console.log(internal_reference_no);
        let company_name="";
        let company_address="";
        ...
        axios({
            ...
            url: api_url,
            headers:headers
          }).then(function (company_api_info){
        // let company_api_info=await axios.get(api_url,{
        //               headers:headers
        //             });
        //console.log(company_api_info);

          /*******************Adding log for Probe api datas store start**************** */   
          let probeApi_data = {
            user_id         :  user_id,
            api_type        :   'comprehensive_api',
            api_name        :   company_type=='cin'?'companies_comprehensive_details':'llps_comprehensive_details',
            api_request        :   '',
            api_params      :       '',
            api_headers     :   JSON.stringify(headers),
            api_result        :  CircularJSON.stringify(company_api_info.data.data),                           
        }


        new ProbeApiDatas().createData(probeApi_data)
        /*******************Adding log for Probe api datas store end**************** */
        let company_api_json='';
        let company_info_formatted='';
        company_api_json=CircularJSON.stringify(company_api_info.data.data);
        company_info_formatted=JSON.parse(company_api_json);
        let dataObj=company_info_formatted;
        //console.log(dataObj,'dataObj');
      ...
      if(company_info_formatted!='')
      {
            ...
            ejsToHtml
                ...
                .then(async function(html){
                    ...
                    //add to probe api table for future prosprect
                    ...
                    //chk if exist then update else insert
                    
                    new CompanyProbeinfo().where('cin_no', company_id).fetch({columns:['id'],require:true})
                    .then(existingprobeinfo=>{
                        //console.log('update');
                        let existingprobeinfo_json=existingprobeinfo.toJSON();
                        //console.log(existingprobeinfo_json);
                        new CompanyProbeinfo({
                            'id': existingprobeinfo_json.id
                        }).save(company_api_data);
                        
                    })
                    .catch((notexist) => {
                        //console.log('add');
                        let api_data_create=new CompanyProbeinfo(company_api_data);
                        api_data_create.save();
                    })
                    // let api_data_create=new CompanyProbeinfo(company_api_data);
                    // if(existingprobeinfo)
                    // {
                    //     //console.log(1);
                    //     let existingprobeinfo_json=existingprobeinfo.toJSON();
                    //     //console.log(existingprobeinfo_json);
                    ...
                    // else
                    // {
                    //     await api_data_create.save({debug:true});
                    // }
    ...
    /**************common fn to fetch comprehensive data and save html CIN */
    llp_comprehensive_report: async function (company_id,user_id,search_investigation_id,online_search_id='',last_updated_date='') 
    {
        // console.log(company_id);
        // console.log(user_id);
        // console.log(search_investigation_id);
        ...
        dataObj.creditRatingList = creditRatingList;
            ...
            ejsToHtml
                ...
                .then(async function(html){
                    ...
                    new CompanyProbeinfo().where('llp_no', company_id).fetch({columns:['id'],require:true})
                    .then(existingprobeinfo=>{
                        //console.log('update');
                        let existingprobeinfo_json=existingprobeinfo.toJSON();
                        //console.log(existingprobeinfo_json);
