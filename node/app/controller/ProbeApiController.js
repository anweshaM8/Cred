...
const ProbeApiController = {
  ...
  /********need to add user id here and will return search inv id */
          ...
          .then(function (response) {
           // let api_result_data=response.data.data.entities;
            ...
            //console.log(api_result_data);
               ...
               /*******************Adding log for Probe api datas store start**************** */
               let probeApi_data = {
                user_id         :user_id,
                api_type        :   'search_api',
                api_name        :   'entities',
    ...
    /***************************search API call and send update req to probe *************/
    getComprehensiveData: async function (req, res, next) {
      ...
      /************update to search investigation table with internal ref n if any********* */
    ...
    /************************************************************************ */
    /************call data status api *****************************/
    let last_updated_date='';
    let api_url_datstatus='';
    try
    {
      if(company_type=='cin')
      {
        api_url_datstatus= process.env.PROBE_API_URL+'/companies/'+company_id+'/datastatus';
        
      }
      else if(company_type=='llp')
      {
        api_url_datstatus= process.env.PROBE_API_URL+'/llps/'+company_id+'/datastatus';
       
      }
       let datastaus_response=await axios({
        method: 'get',
        url: api_url_datstatus,
        headers:headers
      })

      if(datastaus_response)
      {
        let response_data=CircularJSON.stringify(datastaus_response.data);
        let datastaus_res_formatted=JSON.parse(response_data);
        //console.log("Datsattus",datastaus_res_formatted);
        last_updated_date=datastaus_res_formatted.data.data_status.last_details_updated;
        //console.log(last_updated_date);
        //console.log(moment().format('YYYY-MM-DD'));
      }
    }
    catch(error_datastatus)
    {
      return res.status(200).json(res.fnError(error_datastatus, 'Probe data status API error', 400));
    }
    /***************************************************************/
    //as per present requirement always we take it as not updated and will send request for update later it will be based on datastatus
    //0 - not updated 
    //1 - updated already no need to send update req
    let is_updated=1; 
    let is_call_api=0;
    let is_exist_db=0;
    let company_info_formatted='';

      /*****irrespective of last_updated time_stamp we send request to probe for update details***********/
    ...
    if(is_updated==0)
    {
      ...
      if(company_type=='cin')
      {
        api_url= process.env.PROBE_API_URL+'/companies/'+company_id+'/update';
         params = {
          'CinOrPan':company_id
          };
      ...
      {
        api_url= process.env.PROBE_API_URL+'/llps/'+company_id+'/update';
        params = {
          'LlpinOrPan':company_id
          };
    ...
    /*************************************************************************************************/
          ...
          let update_api_info=axios({
            ...
            params: params,
              ...
              let probeApi_data = {
                user_id         :  user_id,
                api_type        :   'update_api',
                api_name        :   company_type=='cin'?'companies_update':'llps_update',
                api_request        :   JSON.stringify(req.query),
                api_params      :  JSON.stringify(params),
                api_headers     :   JSON.stringify(headers),
                api_result        :   CircularJSON.stringify(response.data.data),                           
            }
  
  
            new ProbeApiDatas().createData(probeApi_data)
            /*******************Adding log for Probe api datas store end**************** */ 
  
          
            let update_api_json=CircularJSON.stringify(response.data.data);
            update_api_formatted=JSON.parse(update_api_json);
            update_req_id=update_api_formatted.request_id;
            //console.log("Update Res",update_req_id);

        /*********************insert with update req id into online search row************/
        if(update_req_id!='')
        {
          let set_data={
            user_id:user_id,
            ...
            cin_or_llp_id:company_id,
            is_update_requested:1,
            update_request_id:update_req_id,
            update_requested_on:moment().format('YYYY-MM-DD HH:m:s')
            
          }

          //console.log("req",set_data);

          new OnlineSearchDetails().createData(set_data,{debug:true})             
          .then(async function(response){
          
            return res.status(200).json(res.fnSuccess("" , 'Update Request send', 200 ));
              

          })
          .catch((errors)=>{
  
            return res.status(200).json(res.fnError(errors, 'Probe API error', 400));
      ...
      .catch((errors)=>{
        ...
        return res.status(200).json(res.fnError(errors,'Server error save to online search json',500));
    })
  }
    catch(e)
    {
      return res.status(200).json(res.fnError(e, 'Probe update API error', 400));
    }
    }
    
    //if update not needed directly call comprehensive api and save updated report html
    else{
      try
      {
        
        let res_comprehensive_api='';
        let online_search_id=0;
          if(company_type=='cin')
          {
            res_comprehensive_api=await invoiceGeneration.cin_comprehensive_report(company_id,user_id,search_investigation_id,online_search_id,last_updated_date);
          }
          else
          {
            res_comprehensive_api=await invoiceGeneration.llp_comprehensive_report(company_id,user_id,search_investigation_id,online_search_id,last_updated_date);
          }
          
          
          return res.status(200).json(res.fnSuccess(res_comprehensive_api , 'Response Successfully saved', 200 ));
      }
      catch(e)
      {
  ...
  //   //         /*****************checking for is it updated by last_updated time or not******/
  //   //         //let last_updated_api=company_info_formatted.last_updated;
...
/***************************Update API testing part*********************/
testUpdateReq:  async function (req, res, next) {

  let CinOrPan = "L74120MH1985PLC035308";
  ...
  axios({
      method: 'post',
      url: process.env.PROBE_API_URL+'/companies/'+CinOrPan+'/update',
      headers:headers,
      params: params,
    })
    .then(function (response) {
      let response_data=CircularJSON.stringify(response.data);
      console.log(response_data);
       return res.status(200).json(res.fnSuccess(response_data , 'Response Successfully', 200 ));               
        })
        .catch((errors)=>{     
            // logger.errorLogger.error({
            //     loggedInUserName: 'admin',
