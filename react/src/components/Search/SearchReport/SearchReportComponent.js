...
const SearchReportComponent = (props) => {
    ...
    const submitData = async (data) => {
        ...
        data['draft_html'] = (editorValue) ? editorValue : "";
        console.log(data);

        var editSubmitData = await httpService.setModule('updatereportcontent').update(data);
        hideLoader();
    ...
    return (
        <>
            ...
            {!loderFlag && (Object.keys(editDataList).length) > 0 && <>
                <SearchReportView formSubmitData={formSubmitData}  editDataList={editDataList}  getEditorValue={getEditorValue} />
            </>}
            {!loderFlag && (Object.keys(editDataList).length) == 0 && <>
                <SearchReportView formSubmitData={formSubmitData}  editDataList={editDataList} getEditorValue={getEditorValue}  />
            </>}
