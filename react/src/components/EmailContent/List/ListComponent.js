...
const ListComponent = (props) => {
    ...
    let fetchAPI = async () => {
        showLoader();
        let listDetails = await httpService.setModule('emailContent').search({pagination:true,limit:process.env.REACT_APP_PAGINATION_LIMIT,page:curPage});
    ...
    const checkPermission = async ()=>{
        if(commonJsObj.checkUrlPermission(['read-email-content'])){
            setContentPermission(true);
            fetchAPI();
        }       
        
        if(commonJsObj.checkUrlPermission(['update-email-content'])){
    ...
    return (
        <>
            ...
            {!loderFlag && contentPermission && <>
                <ListView   dataList={dataList}  editContentPermission={editContentPermission}  handlePageChange={handlePageChange} paginationData={paginationData} paginationPageCount={paginationPageCount} curPage={curPage}  />
            </>}
        </>
    )
}
