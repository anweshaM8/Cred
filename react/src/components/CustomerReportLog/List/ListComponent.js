...
const ListComponent = (props) => {
    ...
    let fetchCustomerReportLogApi = async () => {
        ...
        if (listDetails.res.data.status === 'success') {
              ...
              console.log('DATA',listDetails.res.data.data.length)
    ...
    let fetchCustomerInvoiceData = async () => {
        showLoader();
        let listDetails = await httpService.setModule('getInvoiceList').search({pagination:true,limit:process.env.REACT_APP_PAGINATION_LIMIT,page:curPage});
        if (listDetails.res.data.status === 'success') {
    ...
    useEffect(() => {
     ...
     //function to send mail with invoice attachment
     const send_mail = async (invoice_log_id) => {
        let data={};

        data.id=invoice_log_id;
        //console.log(search_investigation_id);
        var editSubmitData = await httpService.setModule('sendmailinvoice').update(data);
        hideLoader();
        if (editSubmitData.res.data.status === 'success') {                                       
    ...
    return (
        <>
            ...
            {!loderFlag && contentPermission && <>
                <ListView dataCustomerReportLogList={dataCustomerReportLogList} months={months} updatePaymentInfo={updatePaymentInfo} paginationData={paginationData} paginationPageCount={paginationPageCount} handlePageChange={handlePageChange} curPage={curPage} send_mail={send_mail}/>
            </>}
        </>
    )
}
